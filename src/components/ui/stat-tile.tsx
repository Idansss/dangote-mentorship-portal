...
function StatTile({ label, value, hint, icon, tone = 'default', className, ...props }: StatTileProps) {
  return (
    <div
      className={cn('rounded-lg border border-border bg-card p-6 shadow-elevation', className)}
      {...props}
    >
      ...
      <div className={cn('mt-2 text-display tabular-nums', toneClass[tone])}>{value}</div>
